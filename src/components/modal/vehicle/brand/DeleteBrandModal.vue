          ...
          <div class="modal-inner" v-show="isOpen" ref="target">
            <div class="modal-content">
              <div class="flex flex-col gap-4">
                <h1 class="text-lg">Please verify the brand before deleting it</h1>
                <div class="flex flex-col gap-2">
                  <h1 class="font-semibold">{{ props.deviceInfo.name }}</h1>
                </div>
              </div>
              <div class="button-wrapper">
                <BaseButton type="button" class="filled__red" :label="delLabel" :loading="deleteBrandIsLoading" @click="onSubmit(props.deviceInfo.id)" />
...
import { useVehicleBrandsStore } from '@/stores/master-data/vehicleBrandsStore'
  ...
  const { status, deleteBrandIsLoading } = storeToRefs(useVehicleBrandsStore())
  ...
  const onSubmit = async (id) => {
    ...
    if (delButtonClick.value == 2) {
      await vehicleBrandsStore.deleteVehicleBrand(id)
