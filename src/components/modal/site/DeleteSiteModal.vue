          ...
          <div class="modal-inner" v-show="isOpen" ref="target">
            <div class="modal-content">
              <div class="flex flex-col gap-4">
                <h1 class="text-lg">Please verify the site before deleting it</h1>
                <div class="flex flex-col gap-2">
                  <h1 class="font-semibold">Name: {{ props.siteInfo.name }}</h1>
                </div>
              </div>
              <div class="button-wrapper">
                <BaseButton type="button" class="filled__red" :label="delLabel" :loading="isLoading" @click="onSubmit(props.siteInfo.id)" />
...
import { onClickOutside } from '@vueuse/core'
  ...
  const sitesStores = useSitesStore()
  ...
  const onSubmit = async (id) => {
    ...
    if (delButtonClick.value == 2) {
      await sitesStores.deleteSite(id)
      modalActive.value = true
      setTimeout(closeNotification, 3000)
      resetButton()
      closeModal()
      sitesStores.getSites()
    }
  }
